...
enyo.makeAbsoluteUrl=function(window,inUrl){
    var c=new RegExp("^([a-z]+:/)?/");
    if(c.test(inUrl)) return inUrl;
    var d=window.location.href.split("/");
    return d.pop(), d.push(inUrl), d.join("/");
}
enyo.fetchAppRootPath=function(){
    return window.document.baseURI.match(new RegExp(".*:\/\/[^#]*\/"))[0];
}
enyo.windows = {
	//* @public
	/**
